...
function sortMatches (season = 0, type = 0, my_char = 0, opp = 0, opp_rank = 0, opp_char = 0, result = 0, ){
    ...
    for (var i in matches){
        if ((matches[i]['season'] == season || season == 0) && (matches[i]['match_type'] == type || type == 0) && (matches[i]['My Character']['char_id'] == my_char || my_char == 0) && (matches[i]['opponent']['opp_id'] == opp || opp == 0) && (matches[i]['rank']['rank_id'] == opp_rank || opp_rank == 0) && (matches[i]['Opponent Character']['char_id'] == opp_char || opp_char == 0) && (matches[i]['result'] == result || result == 0)){
            var match_type = '';
            var win_loss = '';
            var rank_name = '';
            ...
            switch (matches[i]['rank']['rank_id']) {
                case 19:
                    rank_name = "Ult. Grand Master";
                    break;
                default:
                    rank_name = matches[i]['rank']['rank_name'];
            }
